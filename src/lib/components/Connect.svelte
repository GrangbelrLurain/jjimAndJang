...
<article class={`bg-white w-full ${className}`} id="contect">
  <div class="flex flex-col gap-20 py-40">
    <div class="max-w-3xl w-full mx-auto flex flex-col border gap-2 md:p-10 p-5">
    ...
    <div>
      <form class="max-w-screen-md mx-auto md:p-10 p-5 flex flex-col gap-5 border" on:submit={handleSubmit} method="POST">
        <p data-aos="fade-down" class="font-bold">개인정보 수집관련 동의사항</p>
        <p data-aos="fade-down" class="border whitespace-pre-line p-5 text-sm bg-white">
  ...
  -고객님의 정보는 개인정보 보호법에 따라 보호되며 위의 이용목적 외에 별도로 사용하지 않을 것을 약속드립니다.`}
        ...
        <legend class="flex flex-col gap-5">
        ...
        >
        <button data-aos="fade-down" class="btn bg-primary text-white" type="submit">문의하기</button>
      </form>
    </div>
  </div>
</article>
